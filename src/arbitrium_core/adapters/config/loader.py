...
from arbitrium_core.support.dict import deep_merge
...
def _get_validation_schema() -> dict[str, dict[str, Any]]:
    return {
        ...
        "retry": {"required": True, "type": dict, "non_empty": False},
        "features": {"required": True, "type": dict, "non_empty": False},
        "prompts": {"required": True, "type": dict, "non_empty": False},
        "outputs_dir": {"required": True, "type": str, "allow_none": True},
    }
...
class Config:
    ...
    def _load_defaults(self) -> dict[str, Any]:
        from arbitrium_core.support.config.defaults import get_defaults
    ...
    def _merge_with_models_handling(self, user_config: dict[str, Any]) -> None:
        ...
        if "models" not in user_config or not user_config.get("models"):
            ...
            # Don't let empty models: {} wipe the defaults
    ...
    def load_from_dict(
        ...
        if settings:
            ...
            logger.debug(
                f"Merged user settings, final sections: {list(self.config_data.keys())}"
            )
