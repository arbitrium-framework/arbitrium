...
from arbitrium_core.support.constants import (
    DEFAULT_MAX_INSIGHTS,
    INSIGHT_EXTRACTION_PROMPT,
    PLACEHOLDER_RESPONSES,
)
from arbitrium_core.support.logging import get_contextual_logger
from arbitrium_core.support.text import parse_insight_lines
from arbitrium_core.support.validation.response import (
...
class EnhancedKnowledgeBank:
    ...
    def __init__(
        ...
        if similarity_engine is None:
            from arbitrium_core.adapters.similarity import (
    ...
    def _is_duplicate_claim(
        ...
        max_score = max(similarity_scores) > similarity_threshold
        return max_score
    ...
    async def get_top_insights(
        ...
        if num_insights is None:
            return await self.get_all_insights()
        ...
        async with self._lock:
            ...
            return [
                self.insights_db[insight_id] for insight_id in top_insight_ids
            ]

    async def format_insights_for_context(
        self, num_insights: int | None = None
    ) -> str:
