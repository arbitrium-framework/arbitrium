...
class App:
    ...
    def _load_app_config(self) -> Config:
        """
        Loads the application configuration with fallback to config.example.yml.
        """
        ...
        if not config.load():
            # If specified config fails and it's not the example config, try fallback
    ...
    async def _get_app_question(self) -> str:
        ...
        config_question = self.config.config_data.get("question")
        if config_question:
