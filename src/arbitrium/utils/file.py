...
def setup_output_directories(config: dict[str, object]) -> dict[str, bool]:
    """Create all necessary output directories at startup.

    Args:
        config: Configuration dictionary

    Returns:
        Dictionary of directory status (True if successful, False if failed)
    """
    results = {}

    # Get directory paths from config
    reports_dir = str(config.get("reports_dir", "reports"))
    logs_dir = str(config.get("logs_dir", "logs"))
    question_file = str(config.get("question_file", "question.txt"))
    questions_dir = os.path.dirname(question_file)

    # Create each directory
    results["reports"] = ensure_directory_exists(reports_dir)
    results["logs"] = ensure_directory_exists(logs_dir)

    # Only create questions directory if it's not just the current directory
    if questions_dir and questions_dir != ".":
        results["questions"] = ensure_directory_exists(questions_dir)
