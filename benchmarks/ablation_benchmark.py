...
async def run_condition_A(
    arbitrium: Arbitrium, question: str
) -> dict[str, Any]:
    """
...
async def run_condition_B(
    arbitrium: Arbitrium, question: str, best_model_key: str
) -> dict[str, Any]:
    """
...
async def run_condition_C(model_family: str, question: str) -> dict[str, Any]:
    ...
    arbitrium_homo = await Arbitrium.from_config(
        config_path, skip_health_check=False
    )

    logger.info(
        f"Running tournament with {arbitrium_homo.healthy_model_count} instances..."
    )
...
async def run_condition_D(
    ...
    logger.info(
        f"Running tournament with {arbitrium.healthy_model_count} diverse models..."
    )
...
"""
    ...
    return {
        "judge_model": arbitrium.healthy_models[
            judge_model_key
        ].full_display_name,
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run Arbitrium Ablation Benchmark"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    arbitrium_main = await Arbitrium.from_config(
        args.config, skip_health_check=False
    )
    logger.info(
        f"✓ {arbitrium_main.healthy_model_count} healthy models loaded\n"
    )
    ...
    if not args.skip_evaluation:
        evaluation = await evaluate_results(
            all_results, arbitrium_main, args.question
        )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("## Conclusion\n\n")
        f.write(
            "This benchmark validates the core hypotheses of the Arbitrium Framework:\n\n"
        )
