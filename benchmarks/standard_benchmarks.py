...
async def run_arbitrium_on_benchmark(
    ...
    if not arbitrium.is_ready:
        raise RuntimeError(
            f"No healthy models available. Failed: {list(arbitrium.failed_models.keys())}"
        )
...
async def run_benchmark_suite(
    ...
    print(
        "Running baseline (single model) and Arbitrium Framework tournament...\n"
    )
    ...
    tournament_results = await run_arbitrium_on_benchmark(
