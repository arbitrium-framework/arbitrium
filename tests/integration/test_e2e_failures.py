        ...
        class FailAfterInitialModel(MockModel):
            ...
            async def generate(self, prompt: str):
                ...
                if self._call_count > 2:  # After initial responses
                    from arbitrium.models.base import ModelResponse
...
class TestScoreNormalizationEdgeCases:
    ...
    async def test_score_normalization_percentage_scale(
        ...
        normalized = comparison._normalize_score(0.5, "test")
        assert normalized == 5.0  # 0.5 * 10 = 5.0

        normalized = comparison._normalize_score(0.8, "test")
        assert normalized == 8.0  # 0.8 * 10 = 8.0

        normalized = comparison._normalize_score(0.95, "test")
    ...
    async def test_score_normalization_slightly_over_ten(
        ...
        # Test normalizing scores between 10 and 10.5
        normalized = comparison._normalize_score(10.1, "test")
        assert normalized == 1.01  # 10.1 / 10 = 1.01

        normalized = comparison._normalize_score(10.3, "test")
        assert normalized == 1.03  # 10.3 / 10 = 1.03

        normalized = comparison._normalize_score(10.5, "test")
        assert normalized == 1.05  # 10.5 / 10 = 1.05
