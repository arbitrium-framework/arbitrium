.PHONY: help fmt lint test docs clean install dev build publish discover-ollama
...
test-coverage-enforce:  ## Run tests with strict coverage requirements for core modules
	...
	# Core modules require 60% (higher than baseline 50% in pyproject.toml)
	# because they contain critical tournament logic and model abstractions
	python -m pytest tests/ -v \
