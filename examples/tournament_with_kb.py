...
async def main():
    ...
    kb_enabled = arbitrium.config_data.get("knowledge_bank", {}).get("enabled", False)
    ...
    if not kb_enabled:
        ...
        print(f"   Strategy: {arbitrium.config_data['knowledge_bank'].get('strategy', 'extractive')}")
        print(f"   Max entries: {arbitrium.config_data['knowledge_bank'].get('max_entries', 'unlimited')}")
    ...
    if len(metrics["eliminated_models"]) > 0:
        ...
        print(f"   ✅ {len(metrics['eliminated_models'])} models contributed insights before elimination")
        print("   ✅ Surviving models received KB context in improvement rounds")
        print("   ✅ Final answer synthesizes ideas from ALL models, not just the champion")
